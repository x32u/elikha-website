...
const ArtworkCarousel = ({ artworks, onArtworkClick }) => {
  ...
  const renderCard = (artwork, position, colorIndex) => {
    ...
    return (
      ...
      >
        <div
          className="carousel-card__media"
          style={canLoadImage ? { background: `linear-gradient(135deg, ${palette[0]} 0%, ${palette[1]} 52%, ${palette[2]} 100%)` } : undefined}
        >
          {canLoadImage ? (
            <SnapshotThumbnailImage
              src={artwork.image}
              alt={artwork.title}
              className={`carousel-card__img ${isSnapshotDataUri(artwork.image) ? 'carousel-card__img--snapshot' : ''}`}
            />
          ) : (
            <div className={`carousel-card__placeholder color-${(colorIndex % 4) + 1}`}>
        ...
        {position === 'center' && (
          <div className="carousel-card__overlay">
            <span className="carousel-card__title">{artwork.title || 'Untitled Artwork'}</span>
          </div>
        )}
      </button>
    );
    ...
    <div className="artworks-section">
      ...
      <div className="carousel-wrapper">
        <div className="carousel-track">
          {total > 1 && (
            <button type="button" className="carousel-arrow carousel-arrow--left" onClick={handlePrev} aria-label="Previous artwork">
              <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          ...
          {total > 1 && (
            <button type="button" className="carousel-arrow carousel-arrow--right" onClick={handleNext} aria-label="Next artwork">
              <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
